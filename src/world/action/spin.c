...
void action_update_airdash(void) {
    ...
    f32* walkSpeedInitValue = &(&D_800F7B74)[-1]; // index of 0 does not work
    ...
    if (playerStatus->flags & PS_FLAG_ACTION_STATE_CHANGED) {
        ...
        playerSpinState->spinSoundID = 0;
        playerStatus->specialDecorationEffect = NULL;
        playerSpinState->initialSpinTime = 10;
        playerSpinState->fullSpeedSpinTime = 10;
        playerSpinState->spinRate = 80.0f;
        playerSpinState->speedScale = 4.0f;
        playerSpinState->frictionScale = 0.5f;
        playerSpinState->spinCountdown = playerSpinState->initialSpinTime;
        spinFlags = SPIN_TYPE_DEFAULT;
        firstCall = TRUE;
        ...
        fx_sparkles(4, playerStatus->position.x, playerStatus->position.y, playerStatus->position.z, 30.0f);
    ...
    if (playerStatus->actionSubstate >= SUBSTATE_SPIN_2) {
        ...
        if (playerStatus->currentStateTime == 0) {
            ...
            playerStatus->walkSpeed = *walkSpeedInitValue++ * 1.0f;
            playerStatus->runSpeed = *walkSpeedInitValue++ * 1.0f;
    ...
    playerStatus->currentSpeed = 7.0f;
    playerStatus->runSpeed = 7.0f;
